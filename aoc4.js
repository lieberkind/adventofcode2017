...
const unique = arr =>
	arr.reduce((acc, curr) => (acc.indexOf(curr) > -1 ? acc : acc.concat(curr)), []);

const isValid = phrase => {
	const ws = phrase.split(' ');
	return ws.length === unique(ws).length;
};

const isSecondValid = phrase => {
	const ws = phrase.split(' ').map(w =>
		w
			.split('')
			.sort()
			.join('')
...
const first = input.filter(isValid).length;
const second = input.filter(isSecondValid).length;
